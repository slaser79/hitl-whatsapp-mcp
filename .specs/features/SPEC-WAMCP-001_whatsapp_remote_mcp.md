...
| `WHATSAPP_MCP_PORT` | `8089` | port | MCP listener port. MUST differ from the Go bridge's REST port (`127.0.0.1:8080`) to avoid a bind conflict when transport=http/sse with the bridge running; `8089` matches §6.2. |
...
- **2026-05-30 — CoS port-conflict fix (P-IMPL-1 review):** §4.1.1 `WHATSAPP_MCP_PORT`
  default corrected `8080`→`8089`. The Go bridge's REST API already owns
  `127.0.0.1:8080` (`whatsapp-bridge/main.go`, `whatsapp.py` `WHATSAPP_API_URL`),
  so an 8080 MCP listener collides in http/sse mode. 8089 matches the §6.2
  verification example; resolves an internal spec inconsistency. Caught by the
  P-IMPL-1 PR auto-review (a real P1, not a hallucination).
